...
private:
    __m256 __8pxInterpBilinear_Avx2(const float* image, const int image_width, const __m256 xi, const __m256 yi);
};
...
ImgData<T> ImgAlgInterp<T>::_interpBilinear_Avx2(float* coords, int target_width, int target_height)
{
    ...
    float *source_image_ps = new float[source_img_size];
    ...
    for(int i=0;i<target_img_size_r8;i+=8)
    {
        ...
        __m256 ans = __8pxInterpBilinear_Avx2(source_image_ps,this->width_, xi, yi);
