...
public:
    ...
    ImgData<T> transformStd_base(double translate_x, double translate_y, double rotate_rad, double scale, double anchor_x, double anchor_y, int target_width, int target_height);
    ...
    ImgData<T> crop_base(double x0, double y0, int target_width, int target_height, double rotation = 0, double scale = 1);
};
...
template <typename T>
ImgData<T> ImgFuncTransform<T>::transformStd_base(double translate_x, double translate_y, double rotate_rad, double scale, double anchor_x, double anchor_y, int target_width, int target_height)
{
    // 顺序： scale, rotate, translate
    TransformMatrix transform;
    transform.translate(-anchor_x, -anchor_y);
    ...
    transform.scale(scale);
    transform.translate(anchor_x + translate_x, anchor_y + translate_y);
    return this->_transformAffine_Baseline(transform.to_mat3(), target_width, target_height);
}
...
template <typename T>
ImgData<T> ImgFuncTransform<T>::crop_base(double x0, double y0, int target_width, int target_height, double rotation, double scale)
{
    if(rotation==0 && scale==1)
    {
        // 利用仿射变换实现，效率较低，支持扩展功能
    ...
    {
//             逐行拷贝，效率高，功能单纯
        return transformStd_base(-x0, -y0, rotation, scale, this->width_/2, this->height_/2, target_width, target_height);
    }
}
