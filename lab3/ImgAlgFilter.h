...
public:
    ImgAlgFilter() {}
    ImgAlgFilter(const ImgData<T> &img) : ImgData<T>(img) {}
    ImgAlgFilter(ImgData<T> &&img) : ImgData<T>(img) {}
    ...
    static ImgData<float> __getLaplacianKernel2D(int kernel_size)
    {
        ...
        for(int i=0;i<kernel_size;i++) for(int j=0;j<kernel_size;j++) kernel.setPixel(j,i,1);
