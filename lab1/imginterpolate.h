...
class ImgInterpolate: public virtual ImgData<T>
{
    ...
    {
        if(x>=0 && x<this->width_ && y>=0 && y<this->height_) return this->pixel(x,y);
...
public:
    ...
    {
        int x0=floor(x), x1=floor(x)+1, y0=floor(y), y1=floor(y)+1;
        return ((x1-x)*_getPixelWithDefault(x0,y0) +
                (x-x0)*_getPixelWithDefault(x1,y0)) *(y1-y) +
                ((x1-x)*_getPixelWithDefault(x0,y1) +
                (x-x0)*_getPixelWithDefault(x1,y1)) *(y-y0);
    }
};
